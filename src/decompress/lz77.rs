...
pub fn decompress(in_buf: &[u8], out_buf: &mut Vec<u8>) -> ForensicResult<()> {
    ...
    loop {
        if buffered_flag_count == 0 {
            buffered_flags = u32::from_le_bytes(in_buf[input_position..input_position + 4].try_into().unwrap());
            input_position += 4;
        ...
        if (buffered_flags & (1 << buffered_flag_count)) == 0 {
            ...
            let match_bytes = u16::from_le_bytes(in_buf[input_position..input_position + 2].try_into().unwrap()) as u32;
            input_position += 2;
            ...
            if match_length == 7 {
                ...
                if match_length == 15 {
                    ...
                    input_position += 1;
                    if match_length == 255 {
                        match_length = u16::from_le_bytes(in_buf[input_position..input_position + 2].try_into().unwrap()) as u32;
                        input_position += 2;
                        if match_length == 0 {
                            match_length = u32::from_le_bytes(in_buf[input_position..input_position + 4].try_into().unwrap());
                            input_position += 4;
                        }
                        if match_length < 22 {
                            return Err(ForensicError::bad_format_str("decompress_LZ77(): Invalid match length, must be greater than 22"))
                        }
                        match_length -= 22;
                    }
                    match_length += 15;
...
fn basic_lz77_decompression() {
    ...
    let encoded : [u8; 30] = [0x3f,0x00,0x00,0x00,0x61,0x62,0x63,0x64,0x65,0x66,0x67,0x68,0x69,0x6a,0x6b,0x6c,0x6d,0x6e,0x6f,0x70,0x71,0x72,0x73,0x74,0x75,0x76,0x77,0x78,0x79,0x7a];
...
fn basic_lz77_decompression_2() {
    ...
    let encoded : [u8; 13] = [0xff,0xff,0xff,0x1f,0x61,0x62,0x63,0x17,0x00,0x0f,0xff,0x26,0x01];
