...
use forensic_rs::{core::fs::{ChRootFileSystem, StdVirtualFS}, traits::vfs::VirtualFileSystem};

use crate::prefetch::{read_prefetch_file_compressed, read_prefetch_file_no_compressed, read_prefetch_form_fs};
...
fn should_parse_prefetch_v17() {
    ...
    let file = fs.open(Path::new("./artifacts/17/C/Windows/Prefetch/CMD.EXE-087B4001.pf")).unwrap();
...
fn should_parse_prefetch_v30() {
    let mut fs = StdVirtualFS::new();
    let file = fs.open(Path::new("./artifacts/30/C/Windows/Prefetch/CMD.EXE-D269B812.pf")).unwrap();
...
fn should_parse_prefetch_v26() {
    let mut fs = StdVirtualFS::new();
    let file = fs.open(Path::new("./artifacts/26/C/Windows/Prefetch/CMD.EXE-4A81B364.pf")).unwrap();
...
fn should_parse_prefetch_v30_powershell() {
    let mut fs = StdVirtualFS::new();
    let file = fs.open(Path::new("./artifacts/30/C/Windows/Prefetch/POWERSHELL.EXE-AE8EDC9B.pf")).unwrap();
...
fn should_parse_prefetch_v30_cmd() {
    let mut fs = StdVirtualFS::new();
    let file = fs.open(Path::new("./artifacts/30/C/Windows/Prefetch/CMD.EXE-6D6290C5.pf")).unwrap();
